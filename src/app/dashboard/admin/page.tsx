...
import { MOCK_STORES, MOCK_ORDERS } from "@/lib/constants/mock-data";
import {
  ShieldCheck,
  Store,
  Bike,
    ...
    <div className="min-h-screen bg-[#f9fafb] text-[#111827] flex flex-col justify-between font-sans">
      ...
      <main className="mx-auto max-w-7xl w-full px-4 sm:px-6 py-8 sm:py-10 flex-1 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-slate-200 bg-white shadow-sm rounded-2xl p-5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase text-slate-400">Boutiques Partenaires</span>
              <div className="p-2 rounded-xl bg-blue-50 text-blue-600">
                <Store className="w-4 h-4" />
              </div>
            </div>
            <h3 className="text-2xl font-black text-[#111827] mt-3">{stores.length}</h3>
            <p className="text-[11px] text-slate-500 font-medium mt-1">Libreville, Port-Gentil, Franceville</p>
          </Card>

          <Card className="border-slate-200 bg-white shadow-sm rounded-2xl p-5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase text-slate-400">Commandes &amp; Livraisons</span>
              <div className="p-2 rounded-xl bg-purple-50 text-purple-600">
                <ShoppingBag className="w-4 h-4" />
              </div>
            </div>
            <h3 className="text-2xl font-black text-[#111827] mt-3">{orders.length}</h3>
            <p className="text-[11px] text-slate-500 font-medium mt-1">Avec repères visuels</p>
          </Card>
        </div>
