    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-black italic">Espace Coursier &amp; Livreur</h2>
              <Badge variant="blue" className="bg-white/20 text-white border-white/30 text-[10px] uppercase font-black italic">
                Course Express Gabon
              </Badge>
            </div>
            <p className="text-xs text-emerald-100 mt-0.5 font-medium">
              📍 Livraison par repères visuels à Libreville, Akanda, Owendo et en province
            </p>
          </div>
        </div>

        <div className="rounded-xl bg-white/10 p-3 backdrop-blur-md text-right">
          <p className="text-xs text-emerald-100 font-medium">Statut</p>
          <p className="text-base font-black italic text-emerald-300">En service</p>
          <p className="text-[10px] text-emerald-200">Prêt pour les courses</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline">{deliveries.length} active(s)</Badge>
        </div>
        ...
        {deliveries.length === 0 ? (
          <Card className="border border-dashed border-slate-200 bg-white/60 p-12 text-center space-y-3 dark:border-slate-800">
            ...
            <h3 className="text-base font-bold text-slate-900">Aucune course assignée pour le moment</h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Dès qu&apos;une commande marchand est validée et prête à être livrée dans votre secteur, la mission apparaîtra avec l&apos;itinéraire et le point de repère visuel.
            </p>
          </Card>
        ) : (
          deliveries.map((del) => (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                  ...
                  {del.status !== "delivered" && (
                    ...
                    >
                      ...
                      <span>Confirmer la remise du colis</span>
                    </Button>
                  )}
                </div>
              </div>
